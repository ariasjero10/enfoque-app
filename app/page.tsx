...
export default function Home() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0A0A0B] p-6">
        <div className="max-w-md text-center">
          <h1 className="font-bold text-zinc-100 mb-2">Falta configurar Supabase</h1>
          <p className="text-sm text-zinc-400">
            Crea el archivo <code className="text-emerald-400">.env.local</code> a partir de{" "}
            <code className="text-emerald-400">.env.local.example</code> con la URL y la clave anon
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0A0A0B] p-6">
        <div className="max-w-md text-center">
          <h1 className="font-bold text-zinc-100 mb-2">No pudimos conectar con la base de datos</h1>
          <p className="text-sm text-zinc-400 mb-1">Verifica que ejecutaste el script <code>supabase/schema.sql</code> en tu proyecto de Supabase.</p>
          <p className="text-xs text-zinc-500">Detalle: {error}</p>
        </div>
      </div>
    );
  }
